...
const Dashboard = ({ logic }) => {
  ...
  const { user, handleSearch, handleSummarize, handleDeleteBundle, handleDeleteWord, handleUpdateWord, handleDeleteWordFromBundle, handleLinkGoogleAccount, handleRegenerateWord, isLoading, currentSessionWords, pastBundles } = logic;
  ...
  const getToggleBtnClass = (viewName) => {
    ...
    return `relative flex items-center justify-center px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm font-bold rounded-lg transition-all duration-200 focus:outline-none ${isActive
          ...
          <>
            ...
            <div className="max-w-[90rem] mx-auto px-4 sm:px-6 pt-8 pb-20 flex items-start justify-center gap-8">
              ...
              <div className="flex-1 max-w-2xl min-w-0">
                <LiveFeed words={displayedWords} isLoading={isLoading} activeBundleId={activeBundleId} onDeleteWord={handleDeleteWord} onDeleteWordFromBundle={handleDeleteWordFromBundle} onRegenerateWord={handleRegenerateWord} onUpdateWord={handleUpdateWord} />
              </div>

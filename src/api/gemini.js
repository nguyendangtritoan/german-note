...
export const callGeminiApi = async (word, languages, grammarTopic = null, options = {}, promptBuilder = null) => {
  ...
  // const apiKey = import.meta.env.VITE_GEMINI_API_KEY; // REMOVED
  // if (!apiKey) return { error: "Gemini API Key is missing in .env file" }; // REMOVED
  ...
  try {
    ...
    const response = await fetch("/api/gemini", { // UPDATED to local function
      method: 'POST',
